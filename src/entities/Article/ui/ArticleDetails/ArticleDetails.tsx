...
export const ArticleDetails = memo(({ className, articleId }: ArticleDetailsProps) => {
    ...
    if (isLoading) {
        ...
        content = <Text theme={TextTheme.ERROR} text={t('Произошла ошибка при загрузке статьи')} align={TextAlign.CENTER} />;
    } else {
        content = (
            <>
                <Avatar className={cls.avatar} size={200} src={data?.img} />
                <Text className={cls.mainText} title={data?.title} text={data?.subtitle} size={TextSize.L} />
